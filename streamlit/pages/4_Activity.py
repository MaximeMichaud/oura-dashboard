...
import pandas as pd
from components.charts import bar_chart, horizontal_bar, line_chart, pie_chart
from components.metrics import gauge_chart, stat_card
from components.theme import LIGHT_BLUE, ORANGE, RED, SCORE_THRESHOLDS
from data.providers import get_provider

import streamlit as st

st.set_page_config(page_title="Oura - Activity", layout="wide", page_icon=":material/directions_run:")
...
with c1:
    gauge_chart(latest.get("active_calories"), min_val=0, max_val=1500, title="Active Calories", unit=" kcal")
with c2:
...
with c2:
    ...
    if any(v > 0 for v in values):
        fig = pie_chart(
            labels, values, colors=[RED, ORANGE, "#FFBB78", "#AEC7E8", LIGHT_BLUE], title="Activity Breakdown (hours)"
        )
        st.plotly_chart(fig, width="stretch")
...
with c3:
    ...
    if not trend_df.empty:
        fig = line_chart(trend_df, "day", "met", colors=[ORANGE], title="Avg MET Trend", y_label="MET", fill=True)
        st.plotly_chart(fig, width="stretch")
...
with c1:
    contrib_keys = ["Daily Targets", "Move Hourly", "Recovery Time", "Stay Active", "Training Freq", "Training Volume"]
    names = [k for k in contrib_keys if k in latest and latest[k] is not None]
    vals = [latest[k] for k in names]
    if names:
        fig = horizontal_bar(names, vals, thresholds=SCORE_THRESHOLDS, title="Activity Contributors")
        st.plotly_chart(fig, width="stretch")

with c2:
    gauge_chart(latest.get("score"), min_val=0, max_val=100, title="Activity Score", thresholds=SCORE_THRESHOLDS)

with c3:
    if not trend_df.empty:
        fig = line_chart(trend_df, "day", "score", colors=[ORANGE], title="Activity Score Trend")
        st.plotly_chart(fig, width="stretch")
...
with c2:
    if not trend_df.empty:
        fig = line_chart(trend_df, "day", "distance_km", colors=[ORANGE], title="Walking Distance", y_label="km")
        st.plotly_chart(fig, width="stretch")
...
with c1:
    if not trend_df.empty and "target_calories" in trend_df.columns:
        fig = line_chart(
            trend_df,
            "day",
            ["active_calories", "target_calories"],
...
with c2:
    if not trend_df.empty and "steps" in trend_df.columns:
        fig = line_chart(trend_df, "day", "steps", colors=[ORANGE], title="Steps Trend", y_label="steps")
        st.plotly_chart(fig, width="stretch")
...
if not workout_df.empty:
    ...
    with c1:
        if "calories" in workout_df.columns:
            ...
            fig = bar_chart(cal_by_day, "day", "calories", color=ORANGE, title="Workout Calories")
            st.plotly_chart(fig, width="stretch")

    with c2:
        if "activity" in workout_df.columns:
            type_counts = workout_df["activity"].str.replace("_", " ").str.title().value_counts()
            fig = pie_chart(type_counts.index.tolist(), type_counts.values.tolist(), title="Workout Types")
            st.plotly_chart(fig, width="stretch")
    ...
    with c3:
        if "start_datetime" in workout_df.columns and "end_datetime" in workout_df.columns:
            wk = workout_df.copy()
            wk["duration_min"] = (
                pd.to_datetime(wk["end_datetime"]) - pd.to_datetime(wk["start_datetime"])
            ).dt.total_seconds() / 60
            dur_by_day = wk.groupby("day")["duration_min"].sum().reset_index()
            fig = bar_chart(dur_by_day, "day", "duration_min", color=ORANGE, title="Workout Duration (min)")
            st.plotly_chart(fig, width="stretch")
else:
